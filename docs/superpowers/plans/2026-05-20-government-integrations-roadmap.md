...
12. **AI assistant LLM wiring** ✅ shipped 2026-05-21. Replaced the string-template stub in [`AiAssistantController`](../../app/Http/Controllers/AiAssistantController.php) with a real `LlmProvider` contract (`app/Services/Ai/Contracts/LlmProvider.php`) + `AnthropicLlmProvider` backed by the official Anthropic PHP SDK (`anthropic-ai/sdk:^0.23.0`), targeting `claude-haiku-4-5` for cheap/fast summaries. Two prompt-cache breakpoints on the system block (instruction prompt + payload schema/template) — `LlmResponse` returns `cache_read_input_tokens` / `cache_creation_input_tokens` so the dashboard can show savings. `PiiRedactor` is an allow-list projection (employee_no, position, department, status, hire_date month, tenure_years, gender, has_manager) defended by a config blocklist in [`config/ai.php`](../../config/ai.php) — national_id, SSNIT, TIN, bank, salary, phone, address, DOB, emergency contacts all stripped at construction time. `FakeLlmProvider` is a deterministic test double that records every prompt and simulates cache hits, used by default whenever `AI_ENABLED=false` or `ANTHROPIC_API_KEY` is missing (so the UI never 500s on misconfigured tenants — it just shows a clearly-labeled fake reply). 9 new tests across `tests/Feature/Ai/` + `tests/Unit/Ai/` (PII redaction, cache-prefix stability, 503-on-throw, validation).
