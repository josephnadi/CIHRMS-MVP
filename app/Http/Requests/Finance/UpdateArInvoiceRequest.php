...
class UpdateArInvoiceRequest extends FormRequest
{
    ...
    {
        // Same shape as StoreArInvoiceRequest, but the (customer, number)
        // uniqueness check must exclude THIS invoice.
        $currentId = $this->route('arInvoice')?->id;

        $customerInvoiceNoUnique = function (string $attribute, mixed $value, Closure $fail) use ($currentId) {
