...
// ── Executive donut rings ──────────────────────────────────────────
// Replaces the hero's plain inline KPI numbers with three institutional
// health ratios. Each donut maps a raw count to a 0-100 health scale
// against a sensible cap, so the ring fill itself communicates whether
// the metric is at rest, warming, or alarming — colour shifts in step.
//   • Workforce — active headcount against the 1,500 institutional ceiling
//   • Service desk load — open tickets vs the 50-ticket alert threshold
//   • Leave backlog — pending requests vs the 25-request comfort line
const HEADCOUNT_TARGET    = 1500;
const TICKET_ALERT_CAP    = 50;
const LEAVE_BACKLOG_CAP   = 25;

const executiveRings = computed(() => {
    const s = props.stats ?? {};

    const headcount   = Math.max(0, s.employees     ?? 0);
    const tickets     = Math.max(0, s.openTickets   ?? 0);
    const leave       = Math.max(0, s.pendingLeave  ?? 0);

    const headcountPct = Math.min(100, Math.round((headcount / HEADCOUNT_TARGET)  * 100));
    const ticketPct    = Math.min(100, Math.round((tickets   / TICKET_ALERT_CAP)  * 100));
    const leavePct     = Math.min(100, Math.round((leave     / LEAVE_BACKLOG_CAP) * 100));

    // Health-aware colour selection — cool when calm, warm when stressed,
    // red when over threshold. Workforce always stays cyan (it's a capacity
    // gauge, not a stress gauge).
    const stressTone = (p, calm) => p >= 80 ? '#fda4af' : p >= 50 ? '#ffd700' : calm;
    ...
    return [
        {
            ...
            sub:    `of ${HEADCOUNT_TARGET.toLocaleString()} target`,
            pct:    headcountPct,
            color:  '#12d9e3',
            track:  'rgba(18,217,227,0.18)',
        },
        {
            key:    'service',
            label:  'Service load',
            value:  tickets,
            sub:    `cap ${TICKET_ALERT_CAP} tickets`,
            pct:    ticketPct,
            color:  stressTone(ticketPct, '#a7f3d0'),
            track:  'rgba(167,243,208,0.18)',
        },
        {
            key:    'leave',
            label:  'Leave backlog',
            value:  leave,
            sub:    `cap ${LEAVE_BACKLOG_CAP} pending`,
            pct:    leavePct,
            color:  stressTone(leavePct, '#7986cb'),
            track:  'rgba(121,134,203,0.18)',
        },
    ];
});

// SVG donut math shared by the hero rings.
const RING_SIZE = 92;
const RING_STROKE = 9;
const RING_R = (RING_SIZE - RING_STROKE) / 2;
const RING_CIRC = 2 * Math.PI * RING_R;
const ringDashOffset = (pct) => RING_CIRC * (1 - Math.max(0, Math.min(100, pct)) / 100);
                ...
                <div v-if="activeModule === 'overview'" class="space-y-8">
                    ...
                    <section v-if="['super_admin', 'hr_admin'].includes($page.props.auth.user.role)" class="space-y-8 animate-reveal-up">
                        ...
                        <!-- Hero Greeting + Executive Health Rings — the preview-card
                             spot now reads like an executive's status board: salutation
                             + actions on the left, three meaningful donuts on the right
                             that show institutional health at a glance. -->
                        <div class="relative overflow-hidden rounded-3xl px-10 py-8 text-white"
                             style="background:linear-gradient(135deg,#1a237e 0%,#283593 50%,#3949ab 100%);border:1px solid rgba(255,255,255,0.06);">
                            <div class="pointer-events-none absolute -right-16 -top-16 h-72 w-72 rounded-full bg-secondary/10 blur-3xl"></div>
                            <div class="pointer-events-none absolute bottom-0 left-1/3 h-48 w-48 rounded-full blur-2xl" style="background:rgba(255,215,0,0.08);"></div>
                            <!-- Live ribbon -->
                            <div class="absolute inset-x-0 top-0 h-px overflow-hidden">
                                <div class="exec-ribbon h-px w-1/3"></div>
                            </div>
                            ...
                            <div class="relative flex flex-wrap items-center justify-between gap-8">
                                <div class="min-w-0">
                                    ...
                                    <p class="mt-2 text-sm font-medium" style="color:rgba(255,255,255,0.55)">All systems operational — institutional health at a glance.</p>
                                ...
                                <!-- Executive health rings — three institutional ratios
                                     against meaningful caps, with stress-aware colour. -->
                                <div class="flex items-center gap-5 flex-shrink-0">
                                    <div v-for="(ring, i) in executiveRings" :key="ring.key"
                                         class="exec-ring flex flex-col items-center"
                                         :style="`animation:slideUpFade 0.45s cubic-bezier(0.22,1,0.36,1) both;animation-delay:${0.1 + i*0.08}s`">
                                        <div class="relative" :style="`width:${RING_SIZE}px;height:${RING_SIZE}px;`">
                                            <svg :width="RING_SIZE" :height="RING_SIZE" :viewBox="`0 0 ${RING_SIZE} ${RING_SIZE}`"
                                                 class="exec-ring__svg">
                                                <defs>
                                                    <linearGradient :id="`ring-grad-${ring.key}`" x1="0" y1="0" x2="1" y2="1">
                                                        <stop offset="0%"  :stop-color="ring.color" stop-opacity="1"/>
                                                        <stop offset="100%" :stop-color="ring.color" stop-opacity="0.55"/>
                                                    </linearGradient>
                                                </defs>
                                                <!-- Track -->
                                                ...
                                                <!-- Value arc — animated dash, glow shadow -->
                                                <circle :cx="RING_SIZE/2" :cy="RING_SIZE/2" :r="RING_R"
                                                        fill="none"
                                                        :stroke="`url(#ring-grad-${ring.key})`"
                                                        :stroke-width="RING_STROKE"
                                                        stroke-linecap="round"
                                                        :stroke-dasharray="RING_CIRC"
                                                        :stroke-dashoffset="ringDashOffset(ring.pct)"
                                                        :style="`filter:drop-shadow(0 0 4px ${ring.color}66);`"
                                                        class="exec-ring__arc"/>
                                            </svg>
                                            <!-- Center label — large value + percentage strap -->
                                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                                <p class="text-[18px] font-black leading-none tabular-nums kpi-val" :style="`color:${ring.color}`">{{ ring.value }}</p>
                                                <p class="mt-0.5 text-[8.5px] font-black uppercase tracking-[0.15em] tabular-nums" style="color:rgba(255,255,255,0.55)">{{ ring.pct }}%</p>
                                            </div>
                                        </div>
                                        <p class="mt-2 text-[10px] font-black uppercase tracking-[0.18em]" style="color:rgba(255,255,255,0.7)">{{ ring.label }}</p>
                                        <p class="text-[9px]" style="color:rgba(255,255,255,0.35)">{{ ring.sub }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
/* Live ribbon — same streak idiom we use on Service Desk / Messaging
   heroes, so the Executive Console reads as part of the same family. */
.exec-ribbon {
    background: linear-gradient(90deg, transparent, rgba(18,217,227,0.9), rgba(255,215,0,0.7), transparent);
    animation: execRibbon 3.8s linear infinite;
}
@keyframes execRibbon {
