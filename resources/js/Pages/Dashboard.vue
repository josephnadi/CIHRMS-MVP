...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-8">
            ...
            <!-- Benefits & Welfare Module (Employee) -->
            <div v-if="activeModule === 'benefits'" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div class="grid grid-cols-12 gap-8">
                    <!-- Health Insurance Card -->
                    <div class="col-span-12 lg:col-span-8 rounded-3xl border border-outline-variant bg-surface-container-lowest p-8 shadow-sm">
                        <div class="flex items-center justify-between mb-8">
                            <div>
                                <h3 class="text-xl font-black text-primary">Health Insurance & Medical</h3>
                                <p class="text-sm font-medium text-on-surface-variant">Your active institutional coverage details</p>
                            </div>
                            <span class="px-4 py-1.5 rounded-full bg-green-50 text-green-700 border border-green-100 text-[10px] font-black uppercase tracking-widest">Active Coverage</span>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                            <div class="p-6 rounded-2xl bg-surface-container-low/50 border border-outline-variant/30">
                                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-4">Plan Details</p>
                                <div class="space-y-4">
                                    <div class="flex justify-between">
                                        <span class="text-xs font-bold text-primary">Provider</span>
                                        <span class="text-xs font-medium text-on-surface-variant">Nationwide Medical</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-xs font-bold text-primary">Policy #</span>
                                        <span class="text-xs font-mono font-medium text-on-surface-variant">NW-CIHRM-2026</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-xs font-bold text-primary">Tier</span>
                                        <span class="text-xs font-medium text-secondary">Premium Executive</span>
                                    </div>
                                </div>
                            </div>
                            <div class="p-6 rounded-2xl bg-surface-container-low/50 border border-outline-variant/30">
                                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-4">Coverage Scope</p>
                                <ul class="space-y-2">
                                    <li v-for="item in ['In-patient (Full)', 'Out-patient (GHS 5k)', 'Dental & Optical', 'Maternity']" :key="item" class="flex items-center gap-2 text-xs font-medium text-on-surface-variant">
                        ...
                        <div class="mt-8 flex gap-4">
                            <button @click="comingSoon('Insurance e-card download')" type="button" class="flex-1 rounded-xl bg-primary py-3 text-sm font-black text-white shadow-lg shadow-primary/10 hover:bg-primary/90 transition-all">Download E-Card</button>
                            <button @click="showTicketModal = true" type="button" class="flex-1 rounded-xl border border-outline-variant py-3 text-sm font-black text-primary hover:bg-surface-container-low transition-all">Request Reimbursement</button>
                        </div>
                    </div>

                    <!-- Retirement & Savings -->
                    <div class="col-span-12 lg:col-span-4 space-y-8">
                        <div class="rounded-2xl p-8 text-white shadow-xl relative overflow-hidden" style="background:linear-gradient(135deg,#0051d5,#316bf3);">
                            <div class="absolute -right-4 -top-4 opacity-10">
                                <span class="material-symbols-outlined text-9xl">savings</span>
                            </div>
                            <h4 class="text-sm font-black uppercase tracking-widest mb-6 text-white/60">Tier 3 Provident Fund</h4>
                            <div class="mb-8">
                                <p class="text-4xl font-black">GHS 42,150.80</p>
                                <p class="text-[10px] font-medium text-white/50 mt-1 uppercase">Current Accumulated Balance</p>
                            </div>
                            <div class="space-y-4">
                                <div class="flex items-center justify-between text-xs font-bold">
                                    <span class="text-white/70">Monthly Contribution</span>
                                    <span>GHS 850.00</span>
                                </div>
                                <div class="h-1.5 w-full rounded-full bg-surface-container-lowest/10 overflow-hidden">
                                    <div class="h-full w-[45%] bg-surface-container-lowest rounded-full"></div>
                                </div>
                            </div>
                            <button @click="comingSoon('Provident fund investment options')" type="button" class="mt-8 w-full rounded-xl bg-surface-container-lowest/10 py-3 text-sm font-black text-white border border-white/10 hover:bg-white/20 transition-all">
                                Investment Options
                            </button>
                        </div>
                    </div>
